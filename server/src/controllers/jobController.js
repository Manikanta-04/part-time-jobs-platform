...
exports.getJobs = async (req, res) => {
  try {
    ...
    // Handle location filter (city or state)
    if (location) {
      query.$or = [
    ...
    // Handle text search - use regex if text index not available, otherwise use $text
    if (search) {
      ...
      if (query.$or) {
        // If we have location $or, we need to use $and
        query.$and = [
          { $or: query.$or },
          searchQuery,
        ];
